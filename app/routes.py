...
@app.route('/add_cat/<int:user_id>', methods=['GET', 'POST'])
def add_cat(user_id):
    db_sess = db_session.create_session()
    form = AddCat()
    if form.validate_on_submit():
        ...
        posts.male = form.male.data
        posts.breed = form.breed.data
        posts.age = form.age.data
        posts.type_post = form.type_advertisement.data
...
def my_posts(user_id):
    db_sess = db_session.create_session()
    posts = db_sess.query(Posts).filter(Posts.user_id == user_id).all()
    return render_template('my_posts.html', posts=posts)

@app.route('/cat_buy/<int:post_id>')
def cat_buy(post_id):
    db_sess = db_session.create_session()
    post = db_sess.query(Posts).get(post_id)
    return render_template('cat_buy.html', post=post)
